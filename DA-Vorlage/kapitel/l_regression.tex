...
\begin{displaymath}
z(\vec x_i)=\beta_0 + \sum_{j=1}^{d}{\beta_j \cdot x_{ij}} + u_i
\end{displaymath}
...
\min_\beta \sum_{i}^N \log(1+\exp(-(\beta_0+\vec x_i^T\beta))) +C \cdot R(\beta)
\end{displaymath}
Wobei $C \in \mathbb R$ ein Hyperparameter ist,der fest gewählt werden muss. Dies geschieht zumeist durch das Testen verschiedener Werte, sodass hier ein Ansatz zur Parallelisierung entsteht. $R(\beta)$ beschreibt einen Regularisierungsterm, auf den im Kapitel 3.3 näher eingegangen wird.
